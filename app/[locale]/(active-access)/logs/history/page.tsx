...
export default async function HistoryPage({
  ...
  const stats = {
    ...
    approved: sortedLogs.filter((l) => l.status === 'APPROVED').length,
    rejected: sortedLogs.filter((l) => l.status === 'REJECTED').length,
    pending: sortedLogs.filter(
    ...
    <div className='space-y-6'>
      <div>
        ...
        <p className='mt-2 text-muted-foreground'>
          View all completed daily logs from the past
        </p>
      </div>
