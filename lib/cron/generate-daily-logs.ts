...
/**
 * Generate logs for a single schedule on a specific date
 */
export async function generateLogsForSchedule(
  schedule: {
    id: string;
    org_id: string;
    template_id: string;
  ...
  try {
    ...
    for (let i = 0; i < logsToCreate; i++) {
      ...
      if (dailyLog) {
        ...
        console.error(error);
        return { success: logsCreated, failed: 1, skipped: 0, error };
      }
    }

    return { success: logsCreated, failed: 0, skipped: 0 };
  } catch (error) {
    const errorMsg = `Error processing schedule ${schedule.id}: ${
      error instanceof Error ? error.message : String(error)
    }`;
    console.error(errorMsg);
    return { success: 0, failed: 1, skipped: 0, error: errorMsg };
  }
}
...
export async function generateDailyLogsFromSchedules(
  ...
  const results: GenerateLogsResult = {
    success: 0,
    failed: 0,
    skipped: 0,
    errors: [],
  ...
  try {
    ...
    for (const schedule of schedules) {
      const scheduleResult = await generateLogsForSchedule(
        schedule,
        targetDate,
      );
      results.success += scheduleResult.success;
      results.failed += scheduleResult.failed;
      results.skipped += scheduleResult.skipped;
      if (scheduleResult.error) {
        results.errors.push(scheduleResult.error);
      }
    }
