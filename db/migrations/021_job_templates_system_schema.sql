-- Job Templates System - Complete Schema
-- Clean initialization for template-driven job execution with dynamic fields
-- Date: 2026-02-15
...
-- Comments for job_templates
COMMENT ON TABLE job_templates IS 'Master job templates with versioning and organization isolation';
...
-- Indexes for job_template_fields
CREATE INDEX IF NOT EXISTS idx_job_template_fields_template_id ON job_template_fields(template_id);
CREATE INDEX IF NOT EXISTS idx_job_template_fields_category ON job_template_fields(field_category);
CREATE UNIQUE INDEX IF NOT EXISTS idx_job_template_fields_template_key ON job_template_fields(template_id, field_key);

-- Comments for job_template_fields
COMMENT ON TABLE job_template_fields IS 'Dynamic field definitions for templates';
COMMENT ON COLUMN job_template_fields.config_json IS 'Field-specific configuration (options, validation rules, etc.)';
...
CREATE INDEX IF NOT EXISTS idx_jobs_next_execution_date ON jobs(next_execution_date);
CREATE INDEX IF NOT EXISTS idx_jobs_last_execution_date ON jobs(last_execution_date);
CREATE INDEX IF NOT EXISTS idx_jobs_org_id ON jobs(org_id);
CREATE INDEX IF NOT EXISTS idx_jobs_created_by ON jobs(created_by);

-- Comments for jobs
COMMENT ON TABLE jobs IS 'Job instances created from templates with organization isolation';
...
-- Indexes for job_creation_values
CREATE INDEX IF NOT EXISTS idx_job_creation_values_job_id ON job_creation_values(job_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_job_creation_values_job_field ON job_creation_values(job_id, field_key);

-- Comments for job_creation_values
...
-- Indexes for job_actions
CREATE INDEX IF NOT EXISTS idx_job_actions_job_id ON job_actions(job_id);
CREATE INDEX IF NOT EXISTS idx_job_actions_performed_at ON job_actions(performed_at);
CREATE INDEX IF NOT EXISTS idx_job_actions_performed_by ON job_actions(performed_by);

-- Comments for job_actions
...
-- Indexes for job_action_values
CREATE INDEX IF NOT EXISTS idx_job_action_values_action_id ON job_action_values(action_id);

-- Comments for job_action_values
